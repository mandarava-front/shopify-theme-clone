if (!customElements.get('cart-item-options-modal')) {
  class CartItemOptionsModal extends HTMLElement {
    constructor() {
      super();
    ...
    connectedCallback() {
      ...
      this.opener = this.previousElementSibling?.classList.contains('cart-item-options__trigger')
        ? this.previousElementSibling
        : controlsId
          ? document.querySelector(`[aria-controls="${controlsId}"]`)
          : null;
      this.closeButton = this.querySelector('[data-cart-item-options-close]');

      let shouldRestoreOpen = false;
      document.querySelectorAll('cart-item-options-modal').forEach((modal) => {
    ...
    show() {
      ...
      document.querySelectorAll('cart-item-options-modal[open]').forEach((modal) => {
    ...
    restoreFocusTrap() {
      ...
      if (cartDrawer && typeof trapFocus === 'function') {
        const drawerFocusRoot = cartDrawer.classList.contains('is-empty')
          ? cartDrawer.querySelector('.drawer__inner-empty')
          : document.getElementById('CartDrawer');

        if (drawerFocusRoot) {
          trapFocus(drawerFocusRoot, this.opener);
          return;
        }
      }
  ...
  customElements.define('cart-item-options-modal', CartItemOptionsModal);
}
